...
The current version of the project includes an initial evaluation of 10 trajectories generated by a rule-based baseline agent.

Each trajectory was qualitatively evaluated using a manual trace review template and then converted into quantitative scores using a 0-4 rubric.

The current evaluation covers six main dimensions:
...
The aggregated results are stored in `evaluations/results/` and include tables, summaries, and charts generated with Python in English and Spanish.

The script used to generate these results is:
...
- a synthetic dataset of 10 Spanish healthcare-administrative tasks;
- a synthetic knowledge base in English and Spanish;
- a rule-based baseline agent;
- JSON trace generation;
- qualitative trace reviews;
- quantitative rubric-based evaluation;
- Python-generated tables and charts.
